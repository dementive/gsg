...
#define TEST_PRINT_FUNCTION print_line_rich
...
#define SCENE_TEST_PASS_MESSAGE(scene_path) TEST_PRINT_FUNCTION(vformat("\n[color=cyan]Scene Test[/color]: %-43s | [color=green]Passed[/color]", scene_path.replace("res://", "")))

#define TEST_PASS(test_name) TEST_PRINT_FUNCTION(vformat("%-55s | [color=green]Passed[/color]", test_name));
#define TEST_FAIL(test_name, message) TEST_PRINT_FUNCTION(vformat("%-55s | [color=red]Failed[/color] [[color=yellow]%s:%s[/color]] - %s", test_name, __FILE__, __LINE__, message));

// Defines to use in your test conditions and checks
#define VAR_CHECK(first, second) first == Variant(second)
...
#define NULL_CHECK(object)                                                                                                                                                                   \
	if (object == nullptr) {                                                                                                                                                                 \
		TEST_FAIL(vformat("%s nullptr check", #object), vformat("%s is a nullptr!", #object))                                                                                  \
		goto null_##object;                                                                                                                                                                  \
	} \
...
#define TEST_LINE SFT_check_result ? "" : vformat("%s", __LINE__)
...
#define test_scene(scene_path, root_node_class_name, object_name)                                                                                                                            \
	...
	const Ref<PackedScene> scene_ref##_object_name = ResourceLoader::load(scene_path, "PackedScene", ResourceFormatLoader::CACHE_MODE_IGNORE_DEEP);                           \
...
#define named_tests(test_case_name, ...)                                                                                                                                                     \
	TEST_PRINT_FUNCTION(vformat(TEST_CASE_NAME_MESSAGE, test_case_name));                                                                                                             \
