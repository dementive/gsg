...
real_t AStar::_estimate_cost(int64_t p_from_id, int64_t p_end_id) {
...
real_t AStar::_compute_cost(int64_t p_from_id, int64_t p_to_id) {
	Point **from_entry = points.getptr(p_from_id);
...
bool AStar::_solve(Point *begin_point, Point *end_point, bool p_allow_partial_path) {
	...
	while (!open_list.is_empty()) {
		...
		if (last_closest_point == nullptr || last_closest_point->abs_f_score > p->abs_f_score ||
				(last_closest_point->abs_f_score >= p->abs_f_score && last_closest_point->abs_g_score > p->abs_g_score)) {
...
void AStar::add_point(int64_t p_id, const Vector2 &p_pos, real_t p_weight_scale) {
...
Vector2 AStar::get_point_position(int64_t p_id) const {
	Point *const *point_entry = points.getptr(p_id);
	ERR_FAIL_COND_V_MSG(!point_entry, Vector2(), vformat("Can't get point's position. Point with id: %d doesn't exist.", p_id));

	return (*point_entry)->pos;
}

void AStar::set_point_position(int64_t p_id, const Vector2 &p_pos) {
	Point **point_entry = points.getptr(p_id);
...
void AStar::remove_point(int64_t p_id) {
	Point **point_entry = points.getptr(p_id);
...
bool AStar::has_point(int64_t p_id) const { return points.has(p_id); }

Vector<int64_t> AStar::get_point_connections(int64_t p_id) {
...
PackedInt64Array AStar::get_point_ids() {
...
void AStar::set_point_disabled(int64_t p_id, bool p_disabled) {
	Point **p_entry = points.getptr(p_id);
...
bool AStar::is_point_disabled(int64_t p_id) const {
...
void AStar::connect_points(int64_t p_id, int64_t p_with_id, bool bidirectional) {
...
void AStar::disconnect_points(int64_t p_id, int64_t p_with_id, bool bidirectional) {
	Point **a_entry = points.getptr(p_id);
...
bool AStar::are_points_connected(int64_t p_id, int64_t p_with_id, bool bidirectional) const {
	Segment s(p_id, p_with_id);
	const HashSet<Segment, Segment>::Iterator element = segments.find(s);

	return element && (bidirectional || (element->direction & s.direction) == s.direction);
}

int64_t AStar::get_point_count() const { return points.size(); }

int64_t AStar::get_point_capacity() const { return points.get_capacity(); }

void AStar::reserve_space(int64_t p_num_nodes) {
...
int64_t AStar::get_closest_point(const Vector2 &p_point, bool p_include_disabled) const {
...
Vector2 AStar::get_closest_position_in_segment(const Vector2 &p_point) const {
	real_t closest_dist = 1e20;
	...
	for (const Segment &E : segments) {
		...
		Vector2 p = Geometry2D::get_closest_point_to_segment(p_point, from_point->pos, to_point->pos);
...
Vector<Vector2> AStar::get_point_path(int64_t p_from_id, int64_t p_to_id, bool p_allow_partial_path) {
...
Vector<int64_t> AStar::get_id_path(int64_t p_from_id, int64_t p_to_id, bool p_allow_partial_path) {
	Point **a_entry = points.getptr(p_from_id);
