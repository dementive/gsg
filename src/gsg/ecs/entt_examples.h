...
inline void entt_example() {
	...
	if (std::ranges::all_of(provinces_name_view, [&](ProvinceEntity p_entity) { return provinces_name_view.get<Name>(p_entity) == "Ohio"; }))
		print_line("All Ohio\n");
	else
	...
	auto it_found = std::ranges::find_if(provinces_name_view, [&](ProvinceEntity p_entity) { return provinces_name_view.get<Name>(p_entity) == search_string; });
	...
	bool any_california = std::ranges::any_of(provinces_name_view, [&](ProvinceEntity p_entity) { return provinces_name_view.get<Name>(p_entity) == "California"; });

	// Count how many provinces are named "Ohio"
	auto count_ohio = std::ranges::count_if(provinces_name_view, [&](ProvinceEntity p_entity) { return provinces_name_view.get<Name>(p_entity) == "Ohio"; });
