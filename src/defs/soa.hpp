...
/* Macros to put on a struct so SOA data layout isn't a awful to work with
...
#define SOA_INIT(m_type, m_name) \
	m_name.init(data, memory_offsets[current_column]); \
	current_column++;
...
#define SOA_SETGET(m_type, m_name) \
	m_type set_##m_name(Entity p_index) { return m_name[p_index]; } \
	m_type get_##m_name(Entity p_index) { return m_name[p_index]; } \
...
#define FixedSizeSOA(m_total_columns, ...) \
